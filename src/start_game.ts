...
const startGame = (env: Environment) => {
    const camera: [x: number, y: number] = [env.ctx.canvas.width / 2, env.ctx.canvas.height / 2],
        centerCamera = (players: Player[]) => {
            const alivePlayers = players.filter(p => !p.dead),
                averageCoord: [x: number, y: number] =
                    alivePlayers.reduce((avg, p) =>
                        [avg[0] + p.x / alivePlayers.length, avg[1] + p.y / alivePlayers.length],
                        [0, 0]),
    ...
    const players: Player[] = [
        new Default(
            400, 0,
            new Color(255, 0, 0),
            { left: "ArrowLeft", right: "ArrowRight", up: "ArrowUp", down: "ArrowDown", attack: "/", special: "." },
            1
        ),
        new Default(
            -400, 0,
    ...
    for (let i = 0; i < 98; i++) {
        platforms.push(
            new ColoredPlatform(Math.random() * 3000 - 1500, -Math.random() * 9000 + 700, 1000, 15, Color.random(), true)
        );
    }
    ...
    const gameLoop = (before: number) => (now: number) => {
        ...
        players.forEach(p => p.update(dt, { players, platforms, keys: env.keys }));
        players.forEach(p => {
            if (p.y > 1000) {
